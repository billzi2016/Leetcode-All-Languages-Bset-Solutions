...
- LeetCode 1 `Two Sum` 必须有正式流程测试：从 `dataset/merged_problems.json` 读取真实题目，按正式生成流程产出 `easy/1-100/0001-two-sum.md`。
- LeetCode 1 生成完成后必须再跑一次跳过测试：第二次运行应识别该题 `.md` 已完整存在，正常跳过，不重复调用模型。
...
- LeetCode 1 `Two Sum` 能通过正式流程生成，并能在第二次运行时正常跳过。
- 能跳过缺失字段。
- 能断点续跑。
